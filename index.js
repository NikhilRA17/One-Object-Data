...
// For Age:
const AgeSum = objarr.reduce((acc, ele) => {
  acc = acc + ele.age;
  return acc;
}, 0);

// For Names:
const Names = objarr.reduce((acc, ele) => {
  acc.push(ele.name);
  return acc;
}, []);

// For Address:
const Address = objarr.reduce((acc, ele) => {
  acc.push(ele.address);
  return acc;
}, []);

// Output:
console.log({ AgeSum, Address, Names });
